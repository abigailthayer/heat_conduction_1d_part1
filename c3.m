% this code generates an oscillating geotherm T(z) as a movie
...
for i = 1:imax
    T = Ts_bar + DT*exp(-z./zstar_annual).*sin((2*pi*t(i)/P)-(z./zstar_annual)); 
    
    if(rem(t(i),tplot)==0)
        nframe = nframe+1;
    figure(1)
        plot(T,z,'k','linewidth',2) %plot(x-axis,y-axis,color)
        ...
        axis([Ts_bar-DT-1 Ts_bar+DT+1 0 zmax]) %set axis limits
        hold on
